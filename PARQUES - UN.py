...
def crear_esquinas():
    """Crea las esquinas vacías en la matriz para que la ficha vaya por el camino correcto"""
    for i in range(0, 64):
...
def crear_centro():
    """Crea el centro vacío en la matriz para que la ficha vaya por el camino correcto"""
    for i in range(-1, 2):
        for j in range(-1, 2):
            tablero[9+i][9+j] = ['']

def lanzar_dados(dev = False, d1 = 0, d2 = 0):
    """Asigna el valor a los dados ya sea por un lanzamiento o por asignación en el modo desarrollador
    Args:
...
def salir():
    """Saca una ficha de la cárcel dependiendo del jugador actual"""
...
def mover(cantidad, x, y):
    """Mueve una ficha, dada la cantidad de casillas que debe mover y la posición de la ficha
    Args:
        cantidad (int): Cantidad de fichas a mover, 0<=cantidad<=6
        x (int): Posición x de la matriz de la ficha a mover
        y (int): Posición y de la matriz de la ficha a mover
    """
    ...
    esquina3 = False
    esquina4 = False
    # variable para guardar la posición inicial x de la ficha
    ini_x = x
    # variable para guardar la posición inicial y de la ficha
    ini_y = y
    # variable para guardar la última posición x en donde estuvo la ficha
    ant_x = [0]
    # variable para guardar la última posición y en donde estuvo la ficha
    ant_y = [0]
    # preguntar si está en una "recta final" horizontal y no puede moverse porque es un número muy grande
    if x == 9:
        if y < 9:
        ...
        if y > 9:
            if not ('*' in tablero[x][y-cantidad] or (x, y-cantidad) in casillas_especiales['Fin']) or y-cantidad < 9:
                ...
                auto_d2 = False
                return
    # preguntar si esta en una "recta final" vertical y no puede moverse porque es un número muy grande
    if y == 9:
        if x < 9:
    ...
    while cantidad > 0:
        ...
        ant_x.append(x)
        ant_y.append(y)
        # movimiento si está en una casilla donde debe moverse hacia la derecha
        if x == 10:
            if y+1 < len(tablero) and '*' in tablero[x][y+1]:
                ...
                esquina1 = True
        # movimiento si está en una casilla donde debe moverse hacia la izquierda
        elif x == 8:
            if y-1 >= 0 and '*' in tablero[x][y-1]:
                ...
                esquina3 = True
        # movimiento si está en una casilla donde debe moverse hacia abajo
        elif y == 8:
            if x+1 < len(tablero) and '*' in tablero[x+1][y]:
                ...
                esquina4 = True
        # movimiento si está en una casilla donde debe moverse hacia arriba
        elif y == 10:
            if x-1 >= 0 and '*' in tablero[x-1][y]:
                ...
                esquina2 = True
        # movimiento si está en una casilla final y pregunta si es la casilla final del jugador
        elif x == 0:
        ...
        # movimiento si está en una casilla final y pregunta si es la casilla final del jugador
            ...
            if casilla_final[0] == x and casilla_final[1] == y:
                ...
                continue
            x -= 1
        # movimiento si está en una "recta final" horizontal
        elif x == 9:
            if y < 9:
                y += 1
            if y > 9:
                y -= 1
        # movimiento si está en una "recta final" vertical
        elif y == 9:
            if x < 9:
                x += 1
            if x > 9:
        ...
        # movimiento si está en una esquina del trayecto
        if esquina1:
            esquina1 = False
            y += 1
            x += 1
        ...
        # rompe el bucle si encuentra un bloqueo y lo deja en la posición anterior
...
def verificar_salir():
    """Verifica si un jugador puede salir, ya sea por bloqueo, o porque no tiene un 5 o indicar que se salió automáticamente con un 5 que tenga el jugador
    Returns:
        bool
    """
...
def encontrar_jugador(jugador):
    """Dado un jugador retorna su posición
    Args:
...
def verificar_auto():
    """Verifica si la ficha puede moverse automáticamente, de ser asi la mueve"""
...
while ejecutando:
    ...
    for fila in range(len(tablero)):
        for col in range(len(tablero)):
            ...
            # dibuja los nombres de los jugadores debajo de la cárcel
